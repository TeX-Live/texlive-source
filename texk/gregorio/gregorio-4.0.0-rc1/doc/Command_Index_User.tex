...
\begin{tabular}{lp{10cm plus .5cm}}
  ...
  \hline
  debug & Debug messages will be printed to the output log.  Can also be specified as \verb:debug={<types>}:, in which case only messages of the types listed will be printed to the output log.\\
  \hline
...
\textit{Nota bene:} Some font packages (such as \texttt{libertine}) make use of \texttt{fontspec} and thus must also be loaded before \texttt{gregoriotex}.
